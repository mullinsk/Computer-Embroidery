...
if len(lines[0])>30:
	rule1 = "rule BIG {"+ str(math.floor(nlines/100)) +"*{r "+ str(angle) +"} VINE{r 90}} rule BIG .05 {VINE{r 90}}"

rule2 = "rule VINE { SEG{} VINE {x 9.1 y -1.2 r "+str((90-angle)*1.5)+"}}"
...
with open('../cfa/'+fileName+time.strftime('%H%M%S')+'.cfdg', 'w') as outfile: 
	outfile.write(final)
